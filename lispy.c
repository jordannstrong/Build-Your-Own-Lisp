...
void lval_del(lval* v) {
	switch(v->type) {
		...
		case LVAL_SEXPR:
			...
			/* Also free the memory aloocated to contain the pointers */
			free(v->cell);
		break;
	}
...
lval* lval_copy(lval* v) {
	...
	switch(v->type) {
		...
		case LVAL_STR:
			x->str = malloc(strlen(v->str) + 1); strcpy(x->str, v->str);
			break;
...
lval* builtin_all(lenv* e, lval* a) {
	for(int i = 0; i < e->count; i++) {
		printf("Name: %s, Value: %s\n", e->syms[i], lval_eval(e, e->vals[i])->sym); 
	}
...
void lenv_add_builtins(lenv* e) {
	...
	lenv_add_builtin(e, "all", builtin_all);
...
lval* lval_eval_sexpr(lenv* e, lval* v) {
	...
	/* Emsure first element is a function after evaluation */
	lval* f = lval_pop(v, 0);
	if(f->type != LVAL_FUN) {
		lval* err = lval_err(
...
lval* lval_read_num(mpc_ast_t* t) {
	errno = 0;
	double x = (double) atof(t->contents);
	return errno != ERANGE ? lval_num((double) x) : lval_err("Invalid number.");
}
